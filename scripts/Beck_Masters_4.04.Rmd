...
model_path <- "C:/Users/PATH Lab/Box/Models/PCLE Replication"
...
growth_fun <- function(event, trait, RE, match){
  ...
  lapply(k:l, function(x){
    ...
    if(event != "none" & match == "matched"){
      ...
      # get formula
      if(RE == "intslope" & event != "none"){f <- value ~ new.wave*le.group + (new.wave|PROC_SID)} 
      else if (RE == "int" & match == "matched"){f <- value ~ new.wave*le.group + (1|PROC_SID)}
